<?php $__env->startSection('title', 'Breeding Domba'); ?>
<?php $__env->startSection('header-title', 'Program Breeding Domba'); ?>

<?php $__env->startSection('content'); ?>
<div class="space-y-6">
    
    <div class="flex flex-wrap gap-3 justify-end">
        <a href="<?php echo e(route('program.breeding.indukan')); ?>" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md transition transform hover:scale-105">
            <i class="fas fa-female"></i> Indukan
        </a>
        <a href="<?php echo e(route('program.breeding.pejantan')); ?>" class="bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md transition transform hover:scale-105">
            <i class="fas fa-male"></i> Pejantan
        </a>
        <a href="<?php echo e(route('program.breeding.anakan')); ?>" class="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md transition transform hover:scale-105">
            <i class="fas fa-baby-carriage"></i> Anakan
        </a>
        <a href="<?php echo e(route('program.breeding.kawin-ib')); ?>" class="bg-gradient-to-r from-pink-600 to-pink-700 hover:from-pink-700 hover:to-pink-800 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md transition transform hover:scale-105">
        ...
        <div class="overflow-x-auto">
            <table class="custom-table w-full text-sm">
                ...
                <tbody id="anakan-table-body">
                    <tr><td colspan="8" class="text-center py-6 text-gray-400">Memuat data...</td>
                </tbody>
            </table>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="custom-table w-full text-sm">
                ...
                <tbody id="induk-jantan-body">
                    <tr><td colspan="6" class="text-center py-6 text-gray-400">Memuat data...</tr>
                </tbody>
            </table>
        </div>
...
let currentPageInduk = 1;
let currentPageJantan = 1;
...
async function loadIndukTable() {
    try {
        const response = await TernakPark.api.fetchData('/web-api/program/breeding/induk?page=' + currentPageInduk);
        if (response.success) {
            renderTable('induk-betina-body', response.data.induk, [
                { field: 'ear_tag', label: 'Tagging' },
                { field: 'breed_type', label: 'Jenis' },
                { field: 'current_weight', label: 'BB (kg)' },
                { field: 'age_days', label: 'Umur (hari)', suffix: ' hari' },
                { field: 'condition', label: 'Kondisi' },
                { field: 'pen_name', label: 'Kandang' }
            ], true);
            renderPagination('paginationInduk', response.data.pagination, 'induk');
        }
    } catch(e) { console.error(e); }
}

async function loadJantanTable() {
    try {
        const response = await TernakPark.api.fetchData('/web-api/program/breeding/jantan?page=' + currentPageJantan);
        if (response.success) {
            renderTable('induk-jantan-body', response.data.jantan, [
                { field: 'ear_tag', label: 'Tagging' },
                { field: 'breed_type', label: 'Jenis' },
                { field: 'current_weight', label: 'BB (kg)' },
                { field: 'age_days', label: 'Umur (hari)', suffix: ' hari' },
            ...
            renderPagination('paginationJantan', response.data.pagination, 'jantan');
        }
    } catch(e) { console.error(e); }
}
...
function renderTable(tbodyId, data, columns, withDetail) {
    ...
    if (!data || data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="' + (columns.length + (withDetail ? 1 : 0)) + '" class="text-center py-6 text-gray-400">Tidak ada data</td></td>';
        return;
    }
    let html = '';
...
function changePage(type, page) {
    if (type === 'anakan') {
        ...
        currentPageInduk = page;
        loadIndukTable();
    } else if (type === 'jantan') {
        currentPageJantan = page;
        loadJantanTable();
    }
}
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Tugas Akhir\2. Eksperimen Utama Tahap Machine Learning Aplikasi Keseluruhan\Aplikasi_TernakParkWonosalam REST API\ternakpark-frontend\resources\views/program/breeding.blade.php ENDPATH**/ ?>